...
def initialize() {
	...
	state.masterTriggerOn = false
	state.masterTriggerOff = false
...
def handlerSlavesOn(evt) {
	if(!state.masterTriggerOn && evt.isStateChange()) {
...
def handlerSlavesOff(evt) {
	if(!state.masterTriggerOff && evt.isStateChange()) {
